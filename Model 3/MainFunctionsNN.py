...
def setPassLimits(lines, df_pass, home):
    row_stats = df_pass.loc[df_pass['Team'] == home].values[0]
    lines[12 - 1] =  f"#define sidepassLim {str(row_stats[5])}; #define backpassLim {str(row_stats[3])}; var sidepassCount; var backpassCount;"
    #lines[12 - 1] =  f"#define sidepassLim 0; #define backpassLim 0; var sidepassCount; var backpassCount;"

def create_template_files(df_match, df_ratings,season, template_file_3f,template_file_4f_3rows,template_file_4f,template_file_5f,template_file_5f_3rows,template_file_5f_4rows,  match_url_arr,df_pass):
    index = 0
    for ind,row in df_match.iterrows():
        ...
        if (is3levels_home and is3levels_away):
            ...
            setPassLimits(lines, df_pass, home)
            index = index + 1
            ...
            gk_handling_rating = gk_away_ratings_row[0][df_ratings.columns.get_loc('goalkeeping_handling')]
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////         
            ...
            setPassLimits(lines, df_pass, away)
            index = index + 1
            ...
            gk_handling_rating = gk_home_ratings_row[0][df_ratings.columns.get_loc('goalkeeping_handling')]
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////         
            ...
            freekick_middef_home = create_free_kick_rating(midDefIdsHome, df_ratings, home, 'skill_long_passing')     
            freekick_mid_home = create_free_kick_rating(midfielderIdsHome, df_ratings, home, 'skill_fk_accuracy')
            freekick_midfor_home = create_free_kick_rating(midForIdsHome, df_ratings, home, 'skill_fk_accuracy')
            freekick_for_home = create_free_kick_rating(forwardIdsHome, df_ratings, home, 'skill_fk_accuracy')
...
def create_prob_to_lose(id_array, df_ratings, curr_team):
    no_of_players = len(id_array)
    combined_ratings = 0
    marking_ratings = 0
    for ind in range(no_of_players):
        curr_player_ratings_row = df_ratings.loc[(df_ratings['sofifa_id'] == int(float(id_array[ind])))].values
        interception_ratings = curr_player_ratings_row[0][df_ratings.columns.get_loc('mentality_interceptions')]
        standing_tackle_ratings = curr_player_ratings_row[0][df_ratings.columns.get_loc('defending_standing_tackle')]
        sliding_tackle_ratings = curr_player_ratings_row[0][df_ratings.columns.get_loc('defending_sliding_tackle')]
        ...
        marking_rating = curr_player_ratings_row[0][df_ratings.columns.get_loc('defending_marking')]
        if (pd.isna(marking_rating)):
            marking_rating = curr_player_ratings_row[0][df_ratings.columns.get_loc('mentality_positioning')]
        marking_ratings = marking_ratings + int(marking_rating)
    prob_to_lose_curr_team = math.ceil(combined_ratings / no_of_players)
    marking_curr_team = math.ceil(marking_ratings / no_of_players)
    return prob_to_lose_curr_team, marking_curr_team
...
def create_free_kick_rating(id_array, df_ratings, curr_team, rely_skills):
    ...
    return int(round(freekick_def_home / 10, 0))
